...
| 4.If desired, the GCLK_TCx clock can be prescaled via the Prescaler bit group in the Control A register (CTRLA.PRESCALER).– If the prescaler is used, select a prescaler synchronization operation via the Prescaler and Counter Synchronization bit group in the Control A register (CTRLA.PRESYNC).|4.如果需要，GCLK_TCx时钟可以通过控制A中的预分频比特组进行预分频注册（CTRLA.PRESCALER）。- 如果使用预分频器，请通过预分频器选择预分频器同步操作控制寄存器（CTRLA.PRESYNC）中的计数器同步位组。||
