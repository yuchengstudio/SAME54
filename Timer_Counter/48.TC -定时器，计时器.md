...
| 英文 | 中文 | 说明 | 
| :-----| :----- | :----- |
|To enable and use capture operations, the corresponding Capture Channel x Enable bit in the Control A register (CTRLA.CAPTENx) must be written to '1'.|为了使能和使用捕捉操作，控制寄存器（CTRLA.CAPTENx）中相应的捕捉通道x使能位必须写入'1'。||
|A capture trigger can be provided by input event line TC_EV or by asynchronous IO pin WO[x] for each capture channel or by a TC event. To enable the capture from input event line, Event Input Enable bit in the Event Control register (EVCTRL.TCEI) must be written to '1'. To enable the capture from the IO pin,the Capture On Pin x Enable bit in CTRLA register (CTRLA.COPENx) must be written to '1'.|捕捉触发可以由输入事件线路TC_EV或异步IO引脚WO [x]为每个捕捉通道或TC事件提供。 要使能从输入事件线路捕获，事件控制寄存器中的事件输入使能位（EVCTRL.TCEI）必须写入'1'。 要从IO引脚捕获，必须将CTRLA寄存器中的捕获引脚x使能位（CTRLA.COPENx）写入'1'。||
|By default, a capture operation is done when a rising edge is detected on the input signal. Capture on falling edge is available, its activation is depending on the input source: • When the channel is used with a IO pin, write a '1' to the corresponding Invert Enable bit in the Drive Control register (DRVCTRL.INVENx). • When the channel is counting events from the Event System, write a '1' to the TC Event Input InvertEnable bit in Event Control register (EVCTRL.TCINV).|默认情况下，当在输入信号上检测到上升沿时完成捕捉操作。 在下降沿捕捉可用，其激活取决于输入源：•当通道与IO引脚一起使用时，向驱动控制寄存器（DRVCTRL.INVENx）中相应的取反使能位写入1。 •当通道计数来自事件系统的事件时，向事件控制寄存器（EVCTRL.TCINV）中的TC事件输入反转使能位写入1。||
|For input capture, the buffer register and the corresponding CCx act like a FIFO. When CCx is empty or read, any content in CCBUFx is transferred to CCx. The buffer valid flag is passed to set the CCx interrupt flag (IF) and generate the optional interrupt, event or DMA request. The CCBUFx register value can't be read, all captured data must be read from CCx register|对于输入捕捉，缓冲寄存器和相应的CCx像FIFO一样工作。 当CCx为空或读取时，CCBUFx中的任何内容都被传送到CCx。 通过缓冲区有效标志来设置CCx中断标志（IF）并产生可选的中断，事件或DMA请求。 CCBUFx寄存器值无法读取，所有捕获的数据必须从CCx寄存器读取|https://github.com/yuchengstudio/SAME54/blob/master/Timer_Counter/pictures/timer0010.jpg|
|Note: When up-counting (CTRLBSET.DIR=0), counter values lower than 1 cannot be captured. To capture the full range including value 0, the TC must be in down-counting mode (CTRLBSET.DIR=0)|注意：当向上计数（CTRLBSET.DIR = 0）时，不能捕捉到小于1的计数器值。 捕捉全范围包括值0，TC必须处于向下计数模式（CTRLBSET.DIR = 0）||
|By default, a capture operation is done when a rising edge is detected on the input signal. Capture on falling edge is available, its activation is depending on the input source:  • When the channel is used with a IO pin, write a '1' to the corresponding Invert Enable bit in the Drive Control register (DRVCTRL.INVENx). • When the channel is counting events from the Event System, write a '1' to the TC Event Input InvertEnable bit in Event Control register (EVCTRL.TCINV).|默认情况下，当在输入信号上检测到上升沿时完成捕捉操作。 在下降沿捕捉可用，其激活取决于输入源：  •当通道与IO引脚一起使用时，向驱动控制寄存器（DRVCTRL.INVENx）中相应的取反使能位写入1。 •当通道计数来自事件系统的事件时，向事件控制寄存器（EVCTRL.TCINV）中的TC事件输入反转使能位写入1。||
|Event Capture Action:The compare/capture channels can be used as input capture channels to capture events from the Event System and give them a timestamp. The following figure shows four capture events for one capture channel.|事件捕捉动作：比较/捕捉通道可以用作输入捕捉通道来捕捉来自事件系统的事件并给它们一个时间戳。 下图显示了一个捕获通道的四个捕获事件。||
|The TC can detect capture overflow of the input capture channels: When a new capture event is detected while the Capture Interrupt flag (INTFLAG.MCx) is still set, the new timestamp will not be stored and INTFLAG.ERR will be set|TC可以检测输入捕捉通道的捕捉溢出：当捕捉中断标志（INTFLAG.MCx）仍然置位时检测到新的捕捉事件时，新的时间戳不会被存储，并且INTFLAG.ERR将被置位||


![images](https://github.com/yuchengstudio/SAME54/blob/master/Timer_Counter/pictures/timer008.jpg)

![images](https://github.com/yuchengstudio/SAME54/blob/master/Timer_Counter/pictures/timer009.jpg)
...
|Note:  The RETRIGGER, COUNT and START event actions are available only on an event from the Event System|注意：RETRIGGER，COUNT和START事件操作仅适用于来自事件系统的事件||
